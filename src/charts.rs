...
use iced::{
    widget::{button, canvas::Cache},
    Border, Color, Point, Rectangle, Theme, Vector,
};
...
const PRICE_STEPS: [f32; 15] = [
    1000.0, 500.0, 200.0, 100.0, 50.0, 20.0, 10.0, 5.0, 2.0, 1.0, 0.5, 0.2, 0.1, 0.05, 0.01,
];
...
impl canvas::Program<Message> for AxisLabelXCanvas<'_> {
    ...
    fn draw(
        ...
        let (time_step, rounded_earliest) = calculate_time_step(
            earliest_in_millis,
            latest_in_millis,
            x_labels_can_fit,
            self.timeframe,
        ...
        let labels = self.labels_cache.draw(renderer, bounds.size(), |frame| {
            frame.with_save(|frame| {
                ...
                while time <= latest_in_millis {
                    ...
                    if x_position >= 0.0 && x_position <= bounds.width as f64 {
                        ...
                        let label = canvas::Text {
                            ...
                            position: Point::new(
                                x_position as f32 - (text_size * 4.0 / 3.0),
                                bounds.height - 20.0,
                            ),
                ...
                frame.stroke(
                    &line,
                    Stroke::default()
                        .with_color(Color::from_rgba8(81, 81, 81, 0.2))
                        .with_width(1.0),
                );
            });
        });
        ...
        let crosshair = self.crosshair_cache.draw(renderer, bounds.size(), |frame| {
            if self.crosshair && self.crosshair_position.x > 0.0 {
                ...
                let (snap_ratio, text_content) = if let Some(timeframe) = self.timeframe {
                    let crosshair_time =
                        NaiveDateTime::from_timestamp((crosshair_millis / 1000.0) as i64, 0);
                    let crosshair_timestamp = crosshair_time.timestamp();
                    let rounded_timestamp = (crosshair_timestamp as f64 / (timeframe as f64 * 60.0))
                        .round() as i64
                        * timeframe as i64
                        * 60;
                    let rounded_time = NaiveDateTime::from_timestamp(rounded_timestamp, 0);

                    let snap_ratio = (rounded_timestamp as f64 * 1000.0
                    ...
                    let crosshair_time = NaiveDateTime::from_timestamp(
                        (crosshair_millis / 1000.0).floor() as i64,
                        ((crosshair_millis % 1000.0) * 1_000_000.0).round() as u32,
                    );
                    let crosshair_timestamp = crosshair_time.timestamp_millis();

                    let snap_ratio = (crosshair_timestamp as f64 - earliest_in_millis as f64)
                        / (latest_in_millis as f64 - earliest_in_millis as f64);
                    (
                        snap_ratio,
                        crosshair_time
                ...
                let text_background = canvas::Path::rectangle(
                    rectangle_position,
                    Size::new(
                        text_content.len() as f32 * text_size / 2.0 + 2.0 * growth_amount + 1.0,
                        text_size + text_size / 2.0,
                    ),
                );
                        ...
                        if difference_y.abs() > 1.0 {
                            ...
                            *last_position = cursor_position;
                            return (
                                ...
                                Some(Message::YScaling(
                                    (y_scaling * 1000.0).round() / 1000.0,
                                    false,
                                )),
                            );
                        }
    ...
    fn draw(
        ...
        let labels = self.labels_cache.draw(renderer, bounds.size(), |frame| {
            frame.with_save(|frame| {
                ...
                while y <= self.max {
                    let y_position = candlesticks_area_height
                        - ((y - self.min) / y_range * candlesticks_area_height);

                    let text_size = 12.0;
                    let decimal_places = if step < 0.5 {
                        2
                    } else {
                        usize::from(step < 1.0)
                    };
            ...
            .draw(renderer, bounds.size(), |frame| {
                if self.crosshair && self.crosshair_position.y > 0.0 {
                    ...
                    let label_content = format!(
                        ...
                        decimal_places,
                        self.min
                            + (y_range * (candlesticks_area_height - self.crosshair_position.y)
                                / candlesticks_area_height)
                    );

                    let growth_amount = 3.0;
                    let rectangle_position = Point::new(
                        8.0 - growth_amount,
                        self.crosshair_position.y - text_size / 2.0 - 3.0,
                    );
                    let text_position =
                        Point::new(8.0, self.crosshair_position.y - text_size / 2.0 - 3.0);

                    let text_background = canvas::Path::rectangle(
                        rectangle_position,
                        Size::new(
                            label_content.len() as f32 * text_size / 2.0
                                + 2.0 * growth_amount
                                + 4.0,
                            text_size + text_size / 1.8,
                        ),
                    );
                    frame.fill(&text_background, Color::from_rgba8(200, 200, 200, 1.0));
