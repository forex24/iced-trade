...
pub use pane::{PaneContent, PaneSettings, PaneState, Uuid};
use serde::{Deserialize, Serialize};

use crate::{
    charts::{
        candlestick::CandlestickChart, footprint::FootprintChart, heatmap::HeatmapChart,
        timeandsales::TimeAndSales, Message as ChartMessage,
    },
    data_providers::{
...
use iced::{
    ...
    window, Alignment, Element, Length, Point, Size, Task,
};
use std::{
...
impl Dashboard {
        ...
        match message {
            Message::Pane(message) => {
                match message {
                    ...
                    pane::Message::PaneDragged(event) => match event {
                    ...
                    pane::Message::SplitPane(axis, pane) => {
                        let focus_pane = if let Some((new_pane, _)) = self.panes.split(
                            axis,
                            pane,
                            PaneState::new(Uuid::new_v4(), vec![], PaneSettings::default()),
                        ) {
                            Some(new_pane)
                        } else {
                            None
                    ...
                    pane::Message::ChartUserUpdate(message, pane_id) => {
                        match self.update_chart_state(pane_id, message) {
                            ...
                            Err(err) => {
                                return Task::perform(async { err }, move |err: Error| {
                    ...
                    pane::Message::PaneContentSelected(content, pane_id, pane_stream) => {
                        ...
                        let pane_content = match content.as_str() {
                            ...
                            "Footprint chart" => {
                                PaneContent::Footprint(FootprintChart::new(1, 1.0, vec![], vec![]))
                            }
                            "Candlestick chart" => {
                                PaneContent::Candlestick(CandlestickChart::new(vec![], 1))
                            }
                        ...
                        if ["Footprint chart", "Candlestick chart", "Heatmap chart"]
                            ...
                            for stream in pane_stream.iter() {
                                match stream {
                                    StreamType::Kline {
                                        ...
                                        if ["Candlestick chart", "Footprint chart"]
                                            .contains(&content.as_str())
                                        {
                                            tasks.push(create_fetch_klines_task(*stream, pane_id));

                                            if content == "Footprint chart" {
                                                tasks.push(create_fetch_ticksize_task(
                                                    exchange, ticker, pane_id,
                                                ));
                                            }
                                        }
                                    }
                    ...
                    pane::Message::TimeframeSelected(timeframe, pane_id) => {
                        ...
                        match self.set_pane_timeframe(pane_id, timeframe) {
                                    ...
                                    match exchange {
                                        Exchange::BinanceFutures => {
                                            tasks.push(Task::perform(
                                                binance::market_data::fetch_klines(
                                                    *ticker, *timeframe,
                                                )
                                                .map_err(|err| format!("{err}")),
                                                move |klines| {
                                                    Message::FetchEvent(klines, stream, pane_id)
                                                },
                                            ));
                                        }
                                        Exchange::BybitLinear => {
                                            tasks.push(Task::perform(
                                                bybit::market_data::fetch_klines(
                                                    *ticker, *timeframe,
                                                )
                                                .map_err(|err| format!("{err}")),
                                                move |klines| {
                            ...
                            Err(err) => {
                                tasks.push(Task::perform(async { err }, move |err: Error| {
                                    Message::ErrorOccurred(err)
                                }));
                            }
                        }
                ...
                match klines {
                    ...
                    Err(err) => {
                        return Task::perform(async { err }, move |err: String| {
                            Message::ErrorOccurred(Error::FetchError(err))
                        })
                    }
                }
            }
            Message::FetchDistributeKlines(stream_type, klines) => match klines {
                Ok(klines) => {
            ...
            Message::FetchDistributeTicks(stream_type, min_tick_size) => match min_tick_size {
                Ok(ticksize) => {
            ...
            Message::FetchForLayout => {
                ...
                tasks.extend(klines_fetch_all_task(&pane_streams));
                tasks.extend(ticksize_fetch_all_task(&pane_streams));
    ...
    pub fn view<'a>(&'a self) -> Element<'a, Message> {
        ...
        let mut pane_grid = PaneGrid::new(&self.panes, |id, pane, maximized| {
            ...
            pane.view(id, self.panes.len(), is_focused, maximized)
        })
        .spacing(4);
    ...
    pub fn layout_changed(&mut self) -> Task<Message> {
        ...
        Task::perform(async {}, move |_| Message::FetchForLayout)
    }
    ...
    fn set_pane_ticksize(
        ...
        Err(Error::UnknownError(
            "No pane found to change ticksize".to_string(),
        ))
    }
    ...
    fn set_pane_timeframe(
        ...
        Err(Error::UnknownError(
            "No pane found to change tiemframe".to_string(),
        ))
    }
    ...
    pub fn find_and_insert_ticksizes(
        &mut self,
    ...
    fn update_chart_state(
        ...
        Err(Error::UnknownError(
            "No pane found to update its state".to_string(),
        ))
    }
...
fn create_fetch_klines_task(stream: StreamType, pane_id: Uuid) -> Task<Message> {
    match stream {
        StreamType::Kline {
            exchange,
            ticker,
            ...
            Exchange::BybitLinear => Task::perform(
                bybit::market_data::fetch_klines(ticker, timeframe).map_err(|err| format!("{err}")),
                move |klines| Message::FetchEvent(klines, stream, pane_id),
            ),
        },
        _ => Task::none(),
...
fn klines_fetch_all_task(
    stream_types: &HashMap<Exchange, HashMap<Ticker, HashSet<StreamType>>>,
) -> Vec<Task<Message>> {
    ...
    for (exchange, stream) in stream_types {
        ...
        for ticker in ticksize_fetches {
            ...
            match exchange {
                ...
                Exchange::BybitLinear => {
                    let fetch_ticksize = Task::perform(
                        bybit::market_data::fetch_ticksize(ticker).map_err(|err| format!("{err}")),
                        move |ticksize| {
                            Message::FetchDistributeTicks(
                                StreamType::DepthAndTrades { exchange, ticker },
                                ticksize,
