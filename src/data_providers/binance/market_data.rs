...
use crate::data_providers::{
    Depth, FeedLatency, Kline, LocalDepthCache, Order, StreamError, Trade,
};
        ...
        if k == "stream" {
            ...
            match stream_type {
                Some(StreamWrapper::Trade) => {
                    let trade: SonicTrade =
                        sonic_rs::from_str(&v.as_raw_faststr()).context("Error parsing trade")?;

                    return Ok(StreamData::Trade(trade));
                }
                Some(StreamWrapper::Depth) => {
                    let depth: SonicDepth =
                        sonic_rs::from_str(&v.as_raw_faststr()).context("Error parsing depth")?;

                    return Ok(StreamData::Depth(depth));
                }
                Some(StreamWrapper::Kline) => {
                    let kline_wrap: SonicKlineWrap =
                        sonic_rs::from_str(&v.as_raw_faststr()).context("Error parsing kline")?;
...
async fn connect(domain: &str, streams: &str) -> Result<FragmentCollector<TokioIo<Upgraded>>> {
    ...
    let tls_stream: tokio_rustls::client::TlsStream<TcpStream> =
        tls_connector.connect(domain, tcp_stream).await?;
                    ...
                    if let Ok(websocket) = connect(domain, streams.as_str()).await {
                        ...
                        tokio::spawn(async move {
                            ...
                            let depth = match fetched_depth {
                                ...
                                Err(e) => {
                                    log::error!(
                                        "Failed to fetch depth for {}, error: {}",
                                        symbol_str,
                                        e
                                    );
                                    return;
                        ...
                        match rx.await {
                            ...
                            Err(e) => {
                                output
                                    .send(Event::Disconnected(format!(
                                        "Failed to send fetched depth for {}, error: {}",
                                        symbol_str, e
                                    )))
                                    .await
                                    ...
                                    match data {
                                        ...
                                        StreamData::Depth(de_depth) => {
                                                ...
                                                || (last_update_id + 1 > de_depth.final_id)
                                            {
                                                ...
                                                tokio::spawn(async move {
                                                    let fetched_depth =
                                                        fetch_depth(selected_ticker).await;
                                                ...
                                                match rx.await {
                                                    Ok(depth) => orderbook.fetched(depth),
                                                    Err(e) => {
                                            ...
                                            if (prev_id == 0) || (prev_id == de_depth.prev_final_id)
                                            {
                                                ...
                                                let depth_latency =
                                                    chrono::Utc::now().timestamp_millis() - time;
                                                ...
                                                let avg_trade_latency = if !trade_latencies
                                                    .is_empty()
                                                {
                                                    let avg = trade_latencies.iter().sum::<i64>()
                                                        / trade_latencies.len() as i64;
        ...
        loop {
            match &mut state {
                State::Disconnected => {
                    ...
                    if let Ok(websocket) = connect(domain, streams).await {
                        state = State::Connected(websocket);
                        let _ = output.send(Event::Connected(Connection)).await;
                    } else {
                        tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
...
pub async fn fetch_ticksize(ticker: Ticker) -> Result<f32, StreamError> {
    ...
    let tick_size_str = symbol["filters"]
        .as_array()
        ...
        .ok_or_else(|| StreamError::ParseError("PRICE_FILTER not found".to_string()))?["tickSize"]
        .as_str()
